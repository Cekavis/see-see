...
自动化检查证明窗口显示已从创建路径延后到 PNG 解码和背景提交之后，完整回归、发布构建和最终 0.4.3 安装均通过。用户也已目视确认最终安装版符合预期，Windows 端到端验证完成。

## Checks Performed

| Check | Command / Action | Result | Notes |
|-------|------------------|--------|-------|
| Reproduction (post-fix) | 最终 Windows 0.4.3 安装版触发截图 | pass | 用户确认“现在符合预期了” |
...
Close the bug — it is verified end to end on Windows.

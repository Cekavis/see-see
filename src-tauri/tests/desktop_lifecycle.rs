...
#[test]
fn analysis_uses_a_streaming_client_without_changing_model_request_client() {
    ...
    let initial = commands
        ...
        .unwrap()
        .0;
    assert!(initial.contains("let http = providers::streaming_client()?;"));
    ...
    let retry = commands
        ...
        .0;
    let create = retry
        .find("let http = providers::streaming_client()?;")
        .unwrap();

...
export type SettingsApi = {
  ...
  getAppSnapshot?: typeof ipc.getAppSnapshot;
  setSaveHistory?: typeof ipc.setSaveHistory;
};
...
export function Settings({ api = ipc }: { api?: SettingsApi }) {
  ...
  useEffect(() => {
    void refresh();
    if (api.getAppSnapshot)
      void api
        .getAppSnapshot()
        .then((snapshot) => setSaveHistory(snapshot.settings.saveHistory));
  }, [api, refresh]);
  ...
  return (
    <main className="settings-view app-shell">
      ...
      <section className="settings-grid" aria-label="历史设置">
        <label className="toggle">
          <input
            ...
            onChange={(event) => {
              ...
              void api.setSaveHistory?.(value).catch((failure: AppError) => {
                setSaveHistory(!value);
        ...
        <p className="field__hint">
          开启后，原始截图、结果和提示词快照会保存在本机 SQLite
          数据库；关闭后新的分析不会落库。
        </p>
      </section>
            ...
            <article className="config-card" key={config.id}>
              ...
              <div className="button-row">
                ...
                <Button
                  variant="danger"
                  onClick={() => {
                    if (window.confirm(`删除模型配置“${config.name}”？`)) {
                      void api
